                ...
                <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            @if (Auth::user()->role === 'admin')
                                <a href="{{ route('logs.index') }}"
                                    class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-sm text-white tracking-widest hover:bg-gray-700">
                            ...
                            <div x-data="{ open: false }" class="relative inline-block text-left">
                                <button @click="open = !open" type="button"
                                    class="inline-flex items-center justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-sm font-semibold text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-indigo-500 transition ease-in-out duration-150"
                                    id="menu-button">
                                    Buat Dokumen Baru
                                ...
                                <div x-show="open" @click.away="open = false"
                                    ...
                                    <div class="py-1" role="none">
                                        <a href="{{ route('dokumen.create.memo') }}"
                                            class="flex items-center px-4 py-2 text-sm text-indigo-800 hover:bg-indigo-100"
                                            role="menuitem">
                                            <i class="fas fa-file-alt fa-fw mr-2 text-indigo-400"></i>
                                            <span>Buat Memo Internal</span>
                                        </a>
                                        <a href="{{ route('dokumen.create.surat') }}"
                                            class="flex items-center px-4 py-2 text-sm text-indigo-800 hover:bg-indigo-100"
                                            role="menuitem">
                                        ...
                                        @if (in_array(Auth::user()->role, ['admin', 'special']))
                                            <a href="{{ route('dokumen.create.backdate') }}"
                                                class="flex items-center px-4 py-2 text-sm text-indigo-800 hover:bg-indigo-100 border-t border-indigo-200"
                                                role="menuitem">
                    ...
                    <form action="{{ route('dashboard') }}" method="GET"
                        ...
                        <div class="sm:col-span-1">
                            ...
                            <x-select-input id="jenis" name="jenis" class="block mt-1 w-full"
                                onchange="this.form.submit()">
                                <option value="">Semua</option>
                                <option value="memo_internal"
                                    {{ request('jenis') == 'memo_internal' ? 'selected' : '' }}>Memo Internal</option>
                                <option value="surat_keluar" {{ request('jenis') == 'surat_keluar' ? 'selected' : '' }}>
                                    Surat Keluar</option>
                            </x-select-input>
                        </div>
                        <div class="sm:col-span-3">
                            ...
                            <div class="flex mt-1">
                                <x-text-input id="search" class="block w-full rounded-r-none" type="text"
                                    name="search" :value="request('search')" placeholder="Cari perihal atau nomor surat..." />
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full bg-white">
                            <thead class="bg-gray-100">
                                <tr>
                                    @php
                                        $sortLink = fn($orderByField) => route(
                                            'dashboard',
                                    ...
                                    <th
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        <a href="{{ $sortLink('jenis_dokumen') }}" class="hover:underline">Jenis</a>
                                    </th>
                                    <th
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        <a href="{{ $sortLink('nomor_dokumen') }}" class="hover:underline">Nomor
                                            Surat</a>
                                    </th>
                                    <th
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        <a href="{{ $sortLink('created_at') }}" class="hover:underline">Dibuat pada</a>
                                    </th>
                                    <th
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        <a href="{{ $sortLink('perihal') }}" class="hover:underline">Perihal</a>
                                    </th>
                                    <th
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        <a href="{{ $sortLink('perihal') }}" class="hover:underline">Kepada</a>
                                    </th>
                                    <th
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        <a href="{{ $sortLink('order') }}" class="hover:underline">Order</a>
                                    </th>
                                    <th
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        <a href="{{ $sortLink('pic') }}" class="hover:underline">PIC</a>
                                    </th>
                                ...
                                @forelse ($dokumens as $dokumen)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap font-mono">
                                            {{ $dokumen->nomor_dokumen }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            {{ optional(\Carbon\Carbon::parse($dokumen->created_at))->setTimezone('Asia/Jakarta')->format('d/m/Y H:i') ?? '-' }}
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center space-x-2">
                                                ...
                                                {{-- Download PDF --}}
                                                <a href="{{ route('dokumen.pdf', $dokumen->id) }}?preview=1"
                                                    target="_blank"
